...
@Tag(name = "Stories Engine", description = "Endpoints for authoring, updating, publishing, and retrieving stories.")
...
public class StoryController {
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "400", description = "Invalid request payload attributes provided."),
            @ApiResponse(responseCode = "401", description = "Missing or invalid bearer token credentials.")
    })
    @PostMapping
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Story successfully modified and persisted."),
            @ApiResponse(responseCode = "400", description = "Invalid request validation errors."),
            @ApiResponse(responseCode = "403", description = "Access denied. Caller is not the original author."),
            @ApiResponse(responseCode = "404", description = "Target story ID not found in database.")
    })
    @PutMapping("/{storyId}")
    public ResponseEntity<StoryResponse> updateStory(
            Authentication authentication,
            @Parameter(description = "The unique UUID identifier of the story to update") @PathVariable UUID storyId,
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Story status updated to published successfully."),
            @ApiResponse(responseCode = "403", description = "Access denied. Caller is not the original author."),
    ...
    public ResponseEntity<StoryResponse> publishStory(
            Authentication authentication,
            @Parameter(description = "The unique UUID identifier of the story to publish") @PathVariable UUID storyId
                                                     ) {
        UUID authorId = AuthenticatedUser.getId(authentication);
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Story details retrieved successfully."),
            @ApiResponse(responseCode = "403", description = "Access denied. Attempted viewing a private draft without authorship permissions."),
            @ApiResponse(responseCode = "404", description = "Target story ID not found.")
    })
    @GetMapping("/{storyId}")
    ...
    public ResponseEntity<StoryResponse> getStory(
            Authentication authentication,
            @Parameter(description = "The unique UUID identifier of the story to retrieve") @PathVariable UUID storyId
                                                 ) {
        UUID viewerId = null;
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "204", description = "Story removed and archived successfully. No response content returned."),
            @ApiResponse(responseCode = "403", description = "Access denied. Caller is not the original author."),
            @ApiResponse(responseCode = "404", description = "Target story ID not found.")
    })
    ...
    public void deleteStory(
            Authentication authentication,
            @Parameter(description = "The unique UUID identifier of the story to delete") @PathVariable UUID storyId
                           ) {
        UUID authorId = AuthenticatedUser.getId(authentication);
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Paginated workspace list fetched successfully."),
            @ApiResponse(responseCode = "401", description = "Missing or invalid bearer token credentials.")
    })
    @GetMapping("/me")
    public ResponseEntity<Page<StoryResponse>> getMyStories(
            Authentication authentication,
            @Parameter(description = "Optional filter to isolate specific story statuses like DRAFT or PUBLISHED") @RequestParam(required = false) StoryStatus status,
